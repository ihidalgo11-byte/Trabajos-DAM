...
public class LeChuck extends Pirata {
    ...
    public LeChuck(String insultos[], String respuestas[], int vida) {
        ...
        while (insulto1 == insulto2) { // Revisa si los insultos son iguales
            this.insulto2 = random.nextInt(insultos.length);
        }
        this.insulto3 = random.nextInt(insultos.length); // Revisa si los insultos son iguales
        ...
        this.respuestasCorrectas[3] = respuestas[8]; // Posicion insulto personalizado
    }
    ...
    public boolean replica(String respuestaUser) { // Compara la respuesta del usuario con las respuesta correcta del insulto
        return respuestaUser.equals(respuestasCorrectas[posicion]); //Usa posicion de LeChuck y no el de la clase padre
    }
