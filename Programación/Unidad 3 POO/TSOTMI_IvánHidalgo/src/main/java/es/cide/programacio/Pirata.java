...
public class Pirata {
    ...
    private int posicion = random.nextInt(3);
    ...
    // Constructor
    public Pirata(String insultos[], String respuestas[]) {
        this.nom = piratas[random.nextInt(piratas.length)];
        this.vida = random.nextInt(3) + 1; // Vida entre 1 y 3
        this.insultos = insultos;
        this.respuestas = respuestas;
        this.insulto1 = random.nextInt(insultos.length); // Genera un numero aleatorio que indica la posicion del
                                                         // insulto
        this.insulto2 = random.nextInt(insultos.length);
        this.insulto3 = random.nextInt(insultos.length);
        while (insulto1 == insulto2 || insulto1 == insulto3 || insulto2 == insulto3) { // Bucle para que no se repitan
                                                                                       // los insultos
    ...
    // Metodos
    public String insultar() { // Enseña uno de los 3 insultos del pirata
        posicion = random.nextInt(3);
        System.out.println(insultosPropios[posicion]);
        return insultosPropios[posicion];
    }

    public boolean replica(String respuestaUser) { // Compara la respuesta del usuario con las respuesta correcta del
                                                   // insulto
